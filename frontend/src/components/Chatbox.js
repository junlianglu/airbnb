...
const Chatbox = ({ property }) => {
  ...
  const keywords = ["amenities", "wifi", "location", "price"]; // List of clickable keywords

  const handleSendMessage = async (messageContent) => {
    const userMessage = { role: 'user', content: messageContent || input };
    setMessages((prev) => [...prev, userMessage]);
    setInput('');

    // Handle property-specific questions
    if (property && detectPropertyQuery(userMessage.content)) {
      setMessages((prev) => [...prev, { role: 'assistant', content: formatPropertyResponse(userMessage.content) }]);
      return;
    }
  ...
  const handleKeywordClick = (keyword) => {
    handleSendMessage(`Tell me about ${keyword}`);
  };

  function detectPropertyQuery(query) {
    const propertyKeywords = ["amenities", "wifi", "location", "price"];
    return propertyKeywords.some(keyword => query.toLowerCase().includes(keyword));
  }
  
  function formatPropertyResponse(query) {
    if (query.toLowerCase().includes("amenities")) {
      return `The property includes the following amenities: ${property.amenities.join(", ")}.`;
    }
    
    if (query.toLowerCase().includes("wifi")) {
      return property.amenities.includes("WiFi")
        ? "Yes, this property offers WiFi access." 
        : "Unfortunately, this property does not offer WiFi.";
    }
    
    if (query.toLowerCase().includes("location")) {
      return `The property is located at ${property.location}.`;
    }
    
    if (query.toLowerCase().includes("price")) {
      return `The current price for this property is $${property.price} per night.`;
    }
